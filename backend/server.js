...
function generateId(createUsername) {
  ...
  let idExists;
  do {
    newUserId = Math.floor(
      Math.floor(100 + Math.random() * 999) * createUsername.length
    );
  ...
  if (userInUsers) {
    ...
    if (user) {
      ...
      const activeSessionIndex = sessions.findIndex(
        (session) => session.userId === user.id && session.active === true
      );
...
app.post("/me/accounts", (req, res) => {
  ...
  const activeSessionIndex = sessions.findIndex(
    (session) => session.token === token && session.active === true
  );
  if (activeSessionIndex !== -1) {
